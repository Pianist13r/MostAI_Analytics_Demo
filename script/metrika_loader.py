...
Порядок шагов намеренно фиксирован:
  1. visits/hits — сырые данные из Метрики (с дедупликацией после докачки)
  ...
  5. hits_calculated / hit_conversions — зависят от visits_calculated
  6. data_quality — подводим итог после полного обновления
  7. cache warming — прогреваем Redis-кэш Superset
...
from loader_config       import log, VISIT_FIELDS, HIT_FIELDS
...
from loader_hits         import update_hits_calculated
...
def _run_pipeline():
    ...
    download_logs('hits',   HIT_FIELDS,   'metrika_hits',   '"ym:pv:dateTime"')
    _deduplicate_by_id('metrika_hits', '"ym:pv:watchID"')
